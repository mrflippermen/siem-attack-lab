...
def rule(name, query, threshold, severity, comparator=">", window=5):
    body = {
        "name": name,
...
# ---- Cadena RCE (subida -> webshell -> reverse shell) ----
rule("Webshell / RCE via upload",    {"term": {"tags": "webshell"}},          0,  "critical")
rule("Post-exploitation / reverse shell", {"term": {"tags": "rce"}},          0,  "critical")
rule("Privilege escalation (CVE-2025-32463)", {"term": {"tags": "privesc"}},  0,  "critical")
rule("Admin panel / uploader access",{"term": {"tags": "admin_area"}},        0,  "high")

# ---- Respuesta activa acme-shield: 10 ataques / IP / 10 min -> ban ----
#  Umbral alineado con el auto-ban de la app: >=10 peticiones de ataque
#  ('suspicious') en una ventana de 10 min disparan la alerta del SOC,
#  igual que acme-shield banea la IP 10 minutos.
rule("Attack burst >=10 (auto-ban threshold)",
     {"term": {"tags": "suspicious"}}, 9, "critical", comparator=">", window=10)
rule("acme-shield IP auto-banned (10 min)", {"term": {"tags": "ip_banned"}},  0,  "critical")
rule("WAF blocks (acme-shield 403)", {"term": {"tags": "waf_block"}},         4,  "medium")

# ---- Fingerprint de agente automatizado (muerde los cebos anti-IA) ----
rule("AI agent bait triggered", {"term": {"tags": "ai_bait"}},               0,  "low")

# ---- Bloqueo conductual de acme-shield: patron de bot/agente IA -------
#  Se dispara cuando la app detecta rafaga / peticiones complejas seguidas
#  / patron de enumeracion y bloquea la IP (para que "de mas vueltas").
rule("Automated agent throttled (rapid/complex pattern)",
     {"term": {"tags": "bot_throttle"}}, 0, "high")

print("\n✅ SOC provisionado. Reglas activas cada 30s; ventana de 5 min (10 min para el auto-ban).")
