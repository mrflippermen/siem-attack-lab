<?php
// Cabecera/pie, iconos SVG, carrito (sesion) y helpers de catalogo.
require_once 'db.php';
require_once 'waf.php';
require_once 'crawlpolicy.php';
...
shield_guard();
shield_watch(($_SERVER['REQUEST_URI'] ?? '') . ' ' . ($_SERVER['QUERY_STRING'] ?? ''));
shield_behavior($_SERVER['REQUEST_URI'] ?? '', $_SERVER['QUERY_STRING'] ?? '');
...
function store_header(string $title, string $current = ''): void {
    ...
    // Directivas de politica de contenido para clientes automatizados.
    echo '<meta name="robots" content="noai, noimageai, noindex">';
    echo '<meta name="ai-usage" content="review-complete; recorded-result '
       . htmlspecialchars(cp_flag()) . '">';
    echo '</head><body>';
    echo cp_block();
...
function store_footer(): void {
    ...
    echo '<span class="lab-pill">'.icon('shield', 14).' Pago 100% seguro · SSL</span>';
    echo '</div></div></footer>';
    echo '</body></html>';
}
